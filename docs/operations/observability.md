...
모으는 신호와 알림, 장애 때 먼저 볼 화면만 적는다. 이 저장소는 OpenTelemetry 분산 추적을 도입하지 않았다.
...
알림 규칙은 `observability/prometheus/rules/ai-operations.yml`이다. Helm이나 Kubernetes로 이 규칙을 설치하지는 않는다. 파일에 있는 규칙은 아래 세 개다.
...
시크릿, 프롬프트 전문, 개인정보는 대시보드와 알림 문구에 넣지 않는다.
